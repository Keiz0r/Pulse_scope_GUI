...
%- - - - - - - - - - GUI construct - - - - - - - - - - 	
	...
	OUTPUTBOX_outline = uipanel('Parent',f,'Title','Output box','FontSize',12,'Units','normal','Position',[.45,.02,.42,.05],'BackgroundColor',[0.7 0.7 0.7]);
		...
		%EDITABLE VALUES
	SET_value = uicontrol('Parent',Parameters,'Style','edit','Units','normal','string','3','FontSize',12,'Position',[0.016,0.62,0.23,0.2]);
	...
	READ_value = uicontrol('Parent',Parameters,'Style','edit','Units','normal','string','0.3','FontSize',12,'Position',[0.262,0.62,0.23,0.2]);
	...
	RESET_value = uicontrol('Parent',Parameters,'Style','edit','Units','normal','string','-2','FontSize',12,'Position',[0.508,0.62,0.23,0.2]);
	...
	Width_value = uicontrol('Parent',Parameters,'Style','edit','Units','normal','string','150','FontSize',12,'Position',[0.016,0.14,0.23,0.2]);
	...
	scope_timescale_value = uicontrol('Parent',Parameters,'Style','edit','Units','normal','string','20','FontSize',12,'Position',[0.262,0.14,0.23,0.2]);
	...
	Resistance_value = uicontrol('Parent',f,'Style','edit','Units','normal','string','500','FontSize',12,'Position',[0.33,0.02,0.05,0.03]);
	...
	scope_timescale = 20;                                            %Scope timescale uses same units as defined in Pulse_width_units
	SET1 = 3;
	SET2 = SET1;
	RESET1 = -2;                                                    %if <-3.6 problems on next read start. look into generator issues when switching from high negative voltages into positive
	RESET2 = RESET1;
	READ = '0.3';
...
%- - - - - - - - - - - - Functions - - - - - - - - - - - -
    ...
    function setbutton(src,event)
		%- - - - - - - - - - SET - - - - - - - - - -
		[ch1_offset,ch2_offset,ch1_gain,ch2_gain,ch3_gain,ch3_offset] = X_BIG_SET_PROCESS(obj1,obj2,setvalue);                %See function SET_PROCESS
	...
	function readbutton(src,event)
		...
		ch1_data_filtered = filter(filt,1,ch1_data);
		
		plot(ax,x,ch1_data,'y',x,ch3_data,'c',x,ch3_data-ch1_data,'m',x,ch1_data_filtered,'r');
		plot(ax2,(ch3_data(1:ceil(end/2))-ch1_data(1:ceil(end/2))),(ch1_data(1:ceil(end/2))/R_load),'y',(ch3_data(ceil(end/2):end)-ch1_data(ceil(end/2):end)),(ch1_data(ceil(end/2):end)/R_load),'w');
...
%		pause(1)
	...
	function resetbutton(src,event)
		%- - - - - - - - - - RESET - - - - - - - - - -
		[ch1_offset,ch2_offset,ch1_gain,ch2_gain,ch3_gain,ch3_offset] = X_BIG_RESET_PROCESS(obj1,obj2,resetvalue);             %See function RESET_PROCESS
		ch1_data=save_oscilloscope(obj1, 'c1',data_points,ch1_attn,ch1_gain,ch1_offset,ch2_attn,ch2_gain,ch2_offset,ch3_attn,ch3_gain,ch3_offset);
		ch3_data=save_oscilloscope(obj1, 'c3',data_points,ch1_attn,ch1_gain,ch1_offset,ch2_attn,ch2_gain,ch2_offset,ch3_attn,ch3_gain,ch3_offset);
		[R_mem1] = resistance_calculation(ch1_data,ch3_data,R_load);
...
%					end
	...
	%- - - - - - - - - - READ11 - - - - - - - - - -
		...
		measurementnum = measurementnum +1;
		fileno.String = num2str(measurementnum);
		while get(src,'Value') & measurementnum <= (1+4*1000)
		%- - - - - - - - - - RESET - - - - - - - - - -
		[ch1_offset,ch2_offset,ch1_gain,ch2_gain,ch3_gain,ch3_offset] = X_BIG_RESET_PROCESS(obj1,obj2,resetvalue);             %See function RESET_PROCESS
		ch1_data=save_oscilloscope(obj1, 'c1',data_points,ch1_attn,ch1_gain,ch1_offset,ch2_attn,ch2_gain,ch2_offset,ch3_attn,ch3_gain,ch3_offset);
		ch3_data=save_oscilloscope(obj1, 'c3',data_points,ch1_attn,ch1_gain,ch1_offset,ch2_attn,ch2_gain,ch2_offset,ch3_attn,ch3_gain,ch3_offset);
		...
		try_again = 0;
		num_of_sets = 0;
		num_of_sets_measnum = measurementnum;
		while try_again == 0 & get(src,'Value') == 1
		num_of_sets = num_of_sets + 1;
			...
			measurementnum = measurementnum +1;
			fileno.String = num2str(measurementnum);
			if ch1_data_filtered(227) <= (ch1_data_filtered(427)+0.025)
				try_again = 1;
				end
			end
			amount_sets(row,1) = num_of_sets;
			row = row + 1;
			fid = fopen('savedfiles\AmOfSets.txt', 'a+');
			fprintf(fid,'%d\n',amount_sets);
			fid = fclose(fid);
		end
	end
